...
def unclip_recon(x, diffusion_engine, vector_suffix,
                 num_samples=1, offset_noise_level=0.04, device=torch.device("cuda")):
    assert x.ndim==3
    if x.shape[0]==1:
        x = x[[0]]
...
def save_model_merge_conf(config, epoch, model, optimizer, checkpoint_paths, config_merge=None, checkpoint_file_name=None):

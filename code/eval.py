...
# %%
model_ckpt_path = "results/fmri_finetune_NSD_[1]/04-07-2024-16-12-54/checkpoints_50/checkpoint_singlesub_clip_cross_att_NSD_[1]_fmriw0.25_imgw1.5_fmar0.75_imar0.5_fmridl6_imgdl6_pretr1_with_checkpoints_pre_140_doublecontra.pth_epo50_mergconf.pth"
model_image_ckpt_path = "results/fmri_finetune_NSD_[1]/04-07-2024-16-12-54/checkpoints_50/checkpoint_singlesub_clip_cross_att_NSD_[1]_fmriw0.25_imgw1.5_fmar0.75_imar0.5_fmridl6_imgdl6_pretr1_with_checkpoints_pre_140_doublecontra.pth_epo50_mergconf_img.pth"
# %%
...
model = fMRICLIPAutoEncoder(
    config,
    model_image_config,
    config.clip_dim,
    ca_weight=config.fmri_ca_weight,
    skip_weight=config.fmri_skip_weight,
...
model_image = ConditionLDM(
    model_image_config,
    config.clip_dim,
    ca_weight=config.img_ca_weight,
    skip_weight=config.img_skip_weight,
...
for data_iter_step, data_dcit in tqdm(
    enumerate(dataloader_hcp_test), leave=False, total=test_set.num_items
):
    if config.dataset != "NSD":
...
# %%
num_images = min(16, config.batch_size)
generated = model_image.generate_image(
    images[:num_images], fmri_embeddings[:num_images], 50
)
F.to_pil_image(torch.cat([*generated], dim=2))
# %%
F.to_pil_image(torch.cat([*images[:num_images]], dim=2))
# %%
clip_scores = torch.nn.functional.cosine_similarity(
    all_clip_embeddings, all_fmri_embeddings, dim=1
)

print(f"Cosine similarity: {torch.mean(clip_scores)}")  # %%
# %%
...
print(f"CLIP retrieval from fMRI: {cf_retrieval:.3%}")
print(f"fMRI retrieval from CLIP: {fc_retrieval:.3%}")
print(f"Random chance: {1/all_clip_embeddings.shape[0]:.3%}")
# %%
# logit_scale = model_image.image_encoder.logit_scale.exp()
logit_scale = torch.tensor(2.6592).exp()
logits_per_text = (
    torch.matmul(all_clip_embeddings.float(), all_fmri_embeddings.t()) * logit_scale
)
